...
const Reviews = () => {
  ...
  const goToRev = (revIdx: number) => {
    ...
    if (prevInd > revIdx) rightShift();
    else if (revIdx > prevInd) leftShift();
  };

  return (
    <div className="relative mb-[15%] mt-[1%] flex aspect-[4/1] w-screen flex-col items-center md:flex-row md:justify-evenly">
      <motion.div
        ref={rev1}
        variants={shiftVar}
        initial="initial"
        className="absolute right-[66%] -z-10 aspect-[4/3] w-[25vw]"
      >
      ...
      <motion.div
        ...
        initial="initial"
        whileInView="shiftIn"
        className="aspect-[4/3] w-[25vw] cursor-pointer"
        onClick={prevRev}
      >
        <Review quote={reviews[leftInd]?.quote} name={reviews[leftInd]?.name} />
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute left-[67%] -z-10 aspect-[4/3] w-[25vw]"
      >
        <Review
          quote={reviews[rightInd]?.quote}
          name={reviews[rightInd]?.name}
      ...
      <div className="absolute -bottom-[2vw] mx-[10%] flex items-center justify-center space-x-[0.5vw]">
        {reviews.map((_, index) => (
          <motion.div
            key={index}
            ...
            className={`aspect-square w-[1vw] cursor-pointer rounded-full hover:scale-[130%] ${
